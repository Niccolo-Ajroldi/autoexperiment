...
Usage:
    pending_cooldown.py STABLE_PATH DECAY_PATH ITER
...
def main():
    ...
    if len(sys.argv) < 4:
        print("Usage: pending_cooldown.py STABLE_PATH DECAY_PATH ITER", file=sys.stderr)
        print(0)
        return

    # CLI args: LOGS stable_name decay_name iter
    stable_path, decay_path, iter_str = sys.argv[1:]

    # Read and write paths
    read_folder = Path(stable_path)
    write_folder = Path(decay_path)
